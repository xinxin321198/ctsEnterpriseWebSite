...
if (! function_exists ( 'timeDifference' )) {
	function timeDifference($seconds) {
		$times = '';
		$diff = GetMkTime ( time () ) - $seconds;
		$day = floor ( $diff / 86400 );//一天的秒数
		$free = $diff % 86400;//取模
		if ($day > 0) {
			return $day . "天前";
		} else {
...
/**
 * 传入的参数一的时间秒数距离当前的时间差，是否大于参数二的秒数
 * @param     int   $seconds  需要计算的秒数
 *  @param     int   $whetherSeconds  是否大于的秒数
 * @return    boolen
 * 大于返回true,小于返回false
 */
if (! function_exists ( 'isGT' )) {
	function isGT($seconds,$whetherSeconds){
		$diff = GetMkTime ( time () ) - $seconds;//当前秒数减计算的秒数
		if($diff>$whetherSeconds){
			return true;
		}else{
			return false;
		}
